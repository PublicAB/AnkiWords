...
class CardMediaPlayer : Closeable {
    ...
    /**
     * How many times each sound is played during automatic playback.
     *
     * Must be set by callers which have access to preferences
     * (for example: `cardMediaPlayer.automaticPlaybackCount = Prefs.audioAutoplayCount`)
     *
     * Defaults to 1: each sound is played once
     */
    var automaticPlaybackCount: Int = 1
        set(value) {
    ...
    private suspend fun playAllAvTagsInternal(
        ...
        try {
            val playCount = if (isAutomaticPlayback) automaticPlaybackCount else 1
