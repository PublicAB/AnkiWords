...
class ReviewerOptionsFragment :
    ...
    override fun initSubscreen() {
        ...
        // Audio: number of times to auto-play audio
        requirePreference<EditTextPreference>(R.string.audio_autoplay_count_key).apply {
            setOnBindEditTextListener { editText ->
                editText.inputType = InputType.TYPE_CLASS_NUMBER
            }
            // reject non-numeric input (empty or invalid values keep the previous setting)
            setOnPreferenceChangeListener { _, newValue ->
                (newValue as? String)?.toIntOrNull() != null
            }
        }
    }
}
